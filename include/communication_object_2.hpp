...
private: // member types
    ...
    struct domain_id_pair
    {
        ...
        {
            return (first_id < other.first_id ? true : (first_id > other.first_id ? false : (second_id < other.second_id)));
        }
    };
...
public: // member functions
    ...
    [[nodiscard]] handle_type exchange(buffer_info_type<Devices, Fields>... buffer_infos) {
        ...
        detail::for_each(memory_tuple, buffer_info_tuple, [this, &i, &tag_offsets](auto mem, auto bi) {
            ...
            this->allocate<device_type, value_type, typename buffer_memory<device_type>::recv_buffer_type>(
                ...
                [field_ptr](const void *buffer, const index_container_type &c) { field_ptr->unpack(reinterpret_cast<const value_type *>(buffer), c); },
                my_dom_id,
                bi->device_id(),
                tag_offsets[i],
                true);
            this->allocate<device_type, value_type, typename buffer_memory<device_type>::send_buffer_type>(
                ...
                [field_ptr](void *buffer, const index_container_type &c) { field_ptr->pack(reinterpret_cast<value_type *>(buffer), c); },
                my_dom_id,
                bi->device_id(),
                tag_offsets[i],
                false);
    ...
    private : void post(communicator_type &comm)
    {
            ...
            for (auto &f : m_recv_futures)
            {
                if (!m_completed_hooks[k])
                {
                    if (f.ready())
                    {
                        ...
                        if (++completed == m_recv_futures.size())
                            break;
                    }
                }
                ++k;
...
struct test_eq_t<Test> : public std::integral_constant<
                             bool, true>
{
};

template <typename Test, typename T0, typename T1, typename... Ts>
struct test_eq_t<Test, T0, T1, Ts...> : public std::integral_constant<
                                            bool,
